...
def bitfinex_raw_data(period, amount) #period = 5m, 30m, 1h, 2h, 6h \ amount=int : amount of datapoints needed
	...
	@high_prices = []
	open=nil
	HistoricalData.all.last(amount*interval).each_slice(interval).to_a.each do |data_split_array|
...
def bitfinex_prices(period, amount)
	prices = []
	raw_data = bitfinex_raw_data(period, amount)
...
# unless ActiveRecord::Base.connection.table_exists? 'historical_data'
#   puts "ERROR: historical_data table does not exist."
# end

# # puts HistoricalData.create(:close => 5)
# HistoricalData.all.each do |t|
#   puts t.close
# end
# 
...
class Bitfinex
  ...
  @bfx = Bitfinex.new("6HHBe0rx1smq5VhIriMiT4orpAQzBmtcHLn3zojexVj", "VlgAZJ0OVgoonqoEJEddFht8PBDNL3SV3yfqRHUa6ZA")
  #puts bfx.lendbook
  #puts bfx.orderbook
  ...
  #     puts @btc_balance = data["available"].to_f
  #   end
  # end
  # puts bfx.transfer
  # puts bfx.balances
