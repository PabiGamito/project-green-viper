...
@five_min_raw_data = bitfinex_raw_data("30m", 999999)
@five_min_prices = bitfinex_prices("30m", 999999)

puts "Shift?"
shift=gets.chomp.to_i

@thirty_min_raw_data.shift(shift)
@thirty_min_prices.shift(shift)
...
if @short_prices.count == 50
	...
	begin
		...
		#STOP-LOSS SELL
		if @short_raw_data.last["low"].to_f<=@sell_stop_loss && @sell_stop_loss_activate==false #&& atr(@short_raw_data, 1)>2 && stochastic(@short_raw_data.last(50), stochastic_period, stochastic_k, stochastic_d)["k"]<80 #&& atr(@short_raw_data, atr_ma)>1 #&& @short_prices.last(6).first(5).ema>@short_prices.last(5).ema
